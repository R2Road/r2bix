...
namespace r2
{
	...
	template<typename T, typename NodeCleaner = ListNodeCleaner_StayValue<T>>
	class ListNodeAllocatorBasedOnVector
	{
	public:
		using ValueT = T;
		...
		ListNodeAllocatorBasedOnVector() : mContainer(), mNodeCleaner(), mHead( nullptr ), mSize( 0u )
		{
			Clear();
		}
		explicit ListNodeAllocatorBasedOnVector( const SizeT size ) : mContainer(), mNodeCleaner(), mHead( nullptr ), mSize( 0u )
		{
			Reset( size );
		}
		...
		NodeT* Pop()
		{
			R2ASSERT( nullptr != mHead, "Empty : ListNodeAllocatorBasedOnVector::Pop()" );
